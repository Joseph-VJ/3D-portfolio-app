...
const Card3D = ({ item, index, activeIndex, onNext, total, mouseX, mouseY, isPlaying, isMobile = false, swipeOffset = { x: 0, y: 0 }, isFlipped = false, onFlipChange }) => {
  ...
  return (
    <div 
      className={`absolute w-[85vw] h-[80dvh] md:w-[400px] md:h-[700px] card-3d ${isActive ? 'cursor-pointer' : 'pointer-events-none'} ${entranceClass}`}
      style={{
        ...style,
        perspective: '1500px',
...
const App = () => {
  ...
  return (
    ...
    >
      ...
      {/* Animated Scroll Indicator - shows when not on last card */}
      {activeIndex < PORTFOLIO_ITEMS.length - 1 && (
        <div className="fixed bottom-8 left-1/2 -translate-x-1/2 z-40 flex flex-col items-center gap-1 animate-bounce pointer-events-none">
          <div className="text-white/40 text-xs font-bold uppercase tracking-wider">
            {cardFlipped[activeIndex] 
              ? (isMobile ? 'Swipe for Next' : 'Scroll for Next')
              : (isMobile ? 'Swipe to View' : 'Scroll to View')
            }
          </div>
